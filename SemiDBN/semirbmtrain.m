...
%         end
        ...
        for l = 1 : numbatches
            ...
            rbm.vc = opts.momentum * rbm.vc + opts.alpha * sum(h_samples - h_neg)' / opts.batchsize; 
...
%             end 
            ...
            if strcmp(opts.weight_decay,'l2')
                   ...
                   rbm.vC = rbm.vC - rbm.alpha * opts.regularize_c * rbm.C;
            end
            if strcmp(opts.weight_decay,'l1')
                   rbm.vW = rbm.vW - rbm.alpha * opts.regularize * sign(rbm.W);
                   rbm.vC = rbm.vC - rbm.alpha * opts.regularize_c * sign(rbm.C);
            end 
...
%       end
      ...
      if strcmp(opts.approx,'tap2') || strcmp(opts.approx,'tap3') %
           %disp('tap2')
            ...
            buf2 = (h_neg-abs(h_neg).^2)' * (v_neg-abs(v_neg).^2) .* rbm.W; 
...
%     end
    ...
    if strcmp(opts.approx,'tap2') 
      ...
      % vC = vC -0.00005 *rbm.alpha * buf;
        vC = vC - rbm.alpha *0.01* buf;
       
    end
  % rbm.vC = rbm.vC * opts.momentum + vC * rbm.alpha*0.001 /opts.batchsize; % no 1
  %  rbm.vC = rbm.vC * opts.momentum  + vC * rbm.alpha*0.00005 / opts.batchsize;
    rbm.vC = rbm.vC * opts.momentum + vC * rbm.alpha*0.01 /opts.batchsize; 
